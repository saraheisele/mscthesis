...
| `special_pulses/` | Double / wide detection | `double_peaks_detection.py`, `double_peaks_visualization.py`, `prototype_pulse_plots.py` |
...
1. **`special_pulses/double_peaks_detection.py`** *(optional but needed for double/wide)*  
   Applies the supervised PCA + random forest classifier to annotate `.h5` files with
   `is_double_peak`, `is_wide_pulse`, and `special_pulse_class`.  
   ...
   Rule-based detection: `--mode rule-based --detection-mode double|wide`.

2. **`activity_timescales/eel_data_preprocessing.py`**  
   Builds multi-timescale pulse-rate `.npz` histograms.  
   Run three times: `all`, `double`, `wide` (prompted at start).  
...
3. **`activity_timescales/pulse_analysis_plots.py`**  
   ...
   **Output:** `data/processed/{all,double,wide}_pulses/`
